							...
							{{ __('uploaded file') }}
                                       ...
                                       {{ __('pick from') }}
                                        <a class="text-primary bold" href
                                                @click.stop.prevent="googlePicker.show(upload_file)"
                                        >
                                                {{ __('drive') }}
                                        </a>
					{{ __('or attach a') }}
	...
	methods: {
		...
		upload_file(file, i) {
			...
			return new Promise((resolve, reject) => {
				...
				if (file.file_name) {
					form_data.append('filename', file.file_name || file.filename);
				}
...
var googlePicker = {
	...
	init: function() {
		$.getScript(this.apiUrl, () => {
			...
			gapi.load("picker", ()=>{ this.apiLoaded = true; });
		});
	...
	getInstance: function() {
		if (!this.instance) {
			var pickerCallback = (data) => {
                		if (data.action == google.picker.Action.PICKED && this.callback)
                        		this.callback({ file_url: data.docs[0].url.split('?')[0], file_name: data.docs[0].name },0);
        		};
